...
export class GitProviderService implements Disposable {
  private readonly _onDidChangeProviders = new EventEmitter<
    GitProvidersChangeEvent
  >();
  ...
  private _onDidChangeRepositories = new EventEmitter<
    RepositoriesChangeEvent
  >();
  ...
  private readonly _onDidChangeRepository = new EventEmitter<
  ...
  private readonly _pendingRepositories = new Map<
    RepoComparisonKey,
      ...
      !workspace.isTrusted
        ? workspace.onDidGrantWorkspaceTrust(() => {
          if (workspace.isTrusted && workspace.workspaceFolders?.length) {
            void this.discoverRepositories(workspace.workspaceFolders, {
              force: true,
            });
          }
        })
  ...
  private registerCommands(): Disposable[] {
    return [
      registerCommand(
        ...
        () => this.clearAllRepoVisibilityCaches(),
      ),
      registerCommand(
        "gitlens.plus.refreshRepositoryAccess",
        () => this.clearAllOpenRepoVisibilityCaches(),
      ),
    ];
  }
  ...
  @debug<GitProviderService["onWindowStateChanged"]>({
    args: { 0: (e) => `focused=${e.focused}` },
  })
  ...
  get openRepositoryCount(): number {
    return this.repositoryCount === 0
      ? 0
      : count(this.repositories, (r) => !r.closed);
  }
  ...
  register(id: GitProviderId, provider: GitProvider): Disposable {
    if (id !== provider.descriptor.id) {
      throw new Error(
        `Id '${id}' must match provider id '${provider.descriptor.id}'`,
      );
    }
    if (this._providers.has(id)) {
      throw new Error(`Provider '${id}' has already been registered`);
    }
    ...
    const disposable = Disposable.from(
      ...
      provider.onDidChangeRepository(async (e) => {
        ...
        if (
          e.changed(RepositoryChange.Closed, RepositoryChangeComparisonMode.Any)
        ) {
          this.updateContext();

          // Send a notification that the repositories changed
          queueMicrotask(() =>
            this.fireRepositoriesChanged([], [e.repository])
          );
        } else if (
        ...
        if (
          ...
          const visibilityInfo = this.getVisibilityInfoFromCache(
            e.repository.path,
          );
          if (visibilityInfo != null) {
            this.checkVisibilityCachedRemotes(
              e.repository.path,
              visibilityInfo,
              remotes,
            );
  ...
  getOpenRepositories(id: GitProviderId): Iterable<Repository> {
    return filter(
      this.repositories,
      (r) => !r.closed && (id == null || id === r.provider.id),
    );
  }
  ...
  private _discoveredWorkspaceFolders = new Map<
    WorkspaceFolder,
    Promise<Repository[]>
  >();
  ...
  @log<GitProviderService["discoverRepositories"]>({
    args: { 0: (folders) => folders.length },
  })
  async discoverRepositories(
    ...
    try {
      ...
      for (const folder of folders) {
        if (!options?.force && this._discoveredWorkspaceFolders.has(folder)) {
          continue;
        }
      ...
      const repositories = flatMap<
        PromiseFulfilledResult<Repository[]>,
        Repository
      >(
        filter<
          PromiseSettledResult<Repository[]>,
          PromiseFulfilledResult<Repository[]>
        >(
          results,
          (r): r is PromiseFulfilledResult<Repository[]> =>
            r.status === "fulfilled",
        ),
        (r) => r.value,
      );
  ...
  @debug({ exit: true })
  private async discoverRepositoriesCore(
    ...
    try {
      ...
      Logger.error(
        ex,
        `${provider.descriptor.name} Provider(${provider.descriptor.id}) failed discovering repositories in ${
  ...
  private async getSubscription(): Promise<Subscription> {
    return this._subscription ??
      (this._subscription = await this.container.subscription
        .getSubscription());
  }

  private _accessCache:
    & Map<string, Promise<RepoFeatureAccess>>
    & Map<undefined, Promise<FeatureAccess | RepoFeatureAccess>> = new Map();
  async access(
    feature: PlusFeatures | undefined,
    repoPath: string | Uri,
  ...
  async ensureAccess(feature: PlusFeatures, repoPath?: string): Promise<void> {
    ...
    if (allowed === false) {
      throw new AccessDeniedError(subscription.current, subscription.required);
    }
  }
  ...
  private _repoVisibilityCache:
    | Map<string, RepositoryVisibilityInfo>
    | undefined;

  private ensureRepoVisibilityCache(): void {
    if (this._repoVisibilityCache == null) {
      const repoVisibility: [string, RepositoryVisibilityInfo][] | undefined =
  ...
  private clearRepoVisibilityCache(keys?: string[]): void {
    if (keys == null) {
      ...
      const repoVisibility = Array.from(
        this._repoVisibilityCache?.entries() ?? [],
      );
  ...
  @debug<GitProviderService["getVisibilityInfoFromCache"]>({
    exit: (r) => `returned ${r?.visibility}`,
  })
  private getVisibilityInfoFromCache(
    key: string,
  ): RepositoryVisibilityInfo | undefined {
    ...
    const now = Date.now();
    if (
      now - visibilityInfo.timestamp >
        1000 * 60 * 60 * 24 * 30 /* TTL is 30 days */
    ) {
  ...
  private checkVisibilityCachedRemotes(
    ...
    if (visibilityInfo.visibility === "public") {
      if (
        remotes.length == 0 ||
        !remotes.some((r) => r.remoteKey === visibilityInfo.remotesHash)
      ) {
        this.clearRepoVisibilityCache([key]);
        return false;
      }
  ...
  private updateVisibilityCache(
    key: string,
    visibilityInfo: RepositoryVisibilityInfo,
  ): void {
    this.ensureRepoVisibilityCache();
    this._repoVisibilityCache?.set(key, visibilityInfo);
    void this.container.storage.store(
      "repoVisibility",
      Array.from(this._repoVisibilityCache!.entries()),
    );
  }
  ...
  clearAllOpenRepoVisibilityCaches(): void {
    const openRepoProviderPaths = this.openRepositories.map((r) =>
      this.getProvider(r.path).path
    );
  ...
  async visibility(
    repoPath?: string | Uri,
  ): Promise<RepositoriesVisibility | RepositoryVisibility> {
    if (repoPath == null) {
      let visibility = this._reposVisibilityCache;
      if (visibility == null) {
        visibility = await this.visibilityCore();
        if (this.container.telemetry.enabled) {
          this.container.telemetry.setGlobalAttribute(
  ...
  private async visibilityCore(
    ...
    async function getRepoVisibility(
      ...
      if (
        visibilityInfo == null ||
        !this.checkVisibilityCachedRemotes(path, visibilityInfo, remotes)
      ) {
    ...
    if (repoPath == null) {
      ...
      for await (
        const result of asSettled(
          repositories.map((r) => getRepoVisibility.call(this, r.path)),
        )
      ) {
  ...
  private _context: { enabled: boolean; disabled: boolean } = {
    enabled: false,
    disabled: false,
  };
  ...
  async setEnabledContext(enabled: boolean): Promise<void> {
    ...
    if (!enabled && this._initializing) {
      disabled =
        !(this.container.storage.getWorkspace("assumeRepositoriesOnStartup") ??
          false);
    }
    ...
    if (
      this._context.enabled === enabled && this._context.disabled === disabled
    ) return;
    ...
    if (!this._initializing) {
      void this.container.storage.storeWorkspace(
        "assumeRepositoriesOnStartup",
        enabled,
      ).catch();
    }
  }

  private _sendProviderContextTelemetryDebounced:
    | Deferrable<() => void>
    | undefined;
    ...
    async function updateRemoteContext(this: GitProviderService) {
      ...
      if (hasRepositories) {
        for (const repo of this._repositories.values()) {
          ...
          if (
            hasRemotes &&
            ((hasRichRemotes && hasConnectedRemotes) || !integrations)
          ) break;
        }
      }
  ...
  private getProvider(repoPath: string | Uri): GitProviderResult {
    if (
      ...
      (typeof repoPath !== "string" &&
        !this.supportedSchemes.has(repoPath.scheme))
    ) {
      debugger;
  ...
  async getBestRevisionUri(
    ...
    return provider.getBestRevisionUri(
      rp,
      provider.getRelativePath(path, rp),
      ref,
    );
  }
  ...
  getRelativePath(pathOrUri: string | Uri, base: string | Uri): string {
    const { provider } = this.getProvider(
      pathOrUri instanceof Uri ? pathOrUri : base,
    );
  ...
  @log()
  getRevisionUri(
    refOrUri: string | GitUri,
    pathOrFile?: string | GitFile,
    repoPath?: string | Uri,
  ): Uri {
    let path: string;
    ...
    const { provider, path: rp } = this.getProvider(repoPath!);
    return provider.getRevisionUri(
      rp,
      provider.getRelativePath(path, rp),
      ref!,
    );
  ...
  @log<GitProviderService["excludeIgnoredUris"]>({
    args: { 1: (uris) => uris.length },
  })
  ...
  @gate<GitProviderService["fetchAll"]>(
    (repos, opts) =>
      `${repos == null ? "" : repos.map((r) => r.id).join(",")}|${
        JSON.stringify(opts)
      }`,
  )
  @log<GitProviderService["fetchAll"]>({
    args: { 0: (repos) => repos?.map((r) => r.name).join(", ") },
  })
  async fetchAll(
    ...
    await window.withProgress(
      ...
      () =>
        Promise.all(
          repositories!.map((r) => r.fetch({ progress: false, ...options })),
        ),
    );
  }
  ...
  @log<GitProviderService["pullAll"]>({
    args: { 0: (repos) => repos?.map((r) => r.name).join(", ") },
  })
  async pullAll(repositories?: Repository[], options?: { rebase?: boolean }) {
    ...
    await window.withProgress(
      ...
      () =>
        Promise.all(
          repositories!.map((r) => r.pull({ progress: false, ...options })),
        ),
    );
  }
  ...
  @gate<GitProviderService["pushAll"]>((repos) =>
    `${repos == null ? "" : repos.map((r) => r.id).join(",")}`
  )
  @log<GitProviderService["pushAll"]>({
    args: { 0: (repos) => repos?.map((r) => r.name).join(", ") },
  })
  async pushAll(
    repositories?: Repository[],
    ...
    await window.withProgress(
      ...
      () =>
        Promise.all(
          repositories!.map((r) => r.push({ progress: false, ...options })),
        ),
    );
  }

  @log<GitProviderService["getAheadBehindCommitCount"]>({
    args: { 1: (refs) => refs.join(",") },
  })
  ...
  @log<GitProviderService["getBlameForLine"]>({
    args: { 2: (d) => d?.isDirty },
  })
  /**
  ...
  @log<GitProviderService["getBlameForLineContents"]>({
    args: { 2: "<contents>" },
  })
  /**
  ...
  async getBlameForRange(
    uri: GitUri,
    range: Range,
  ): Promise<GitBlameLines | undefined> {
    const { provider } = this.getProvider(uri);
    return provider.getBlameForRange(uri, range);
  }

  @log<GitProviderService["getBlameForRangeContents"]>({
    args: { 2: "<contents>" },
  })
  async getBlameForRangeContents(
    uri: GitUri,
    range: Range,
    contents: string,
  ): Promise<GitBlameLines | undefined> {
  ...
  @log<GitProviderService["getBranchAheadRange"]>({
    args: { 0: (b) => b.name },
  })
  async getBranchAheadRange(branch: GitBranch): Promise<string | undefined> {
    ...
    if (branch.upstream == null) {
      ...
      if (branches.length > 0) {
        ...
        const possibleBranch = weightedBranch!.branch.upstream?.name ??
          weightedBranch!.branch.ref;
  ...
  > {
    ...
    const branchesAndTagsBySha = groupByFilterMap(
      (branches as (GitBranch | GitTag)[]).concat(
        tags as (GitBranch | GitTag)[],
      ),
      (bt) => bt.sha,
      (bt) => {
        if (currentName) {
          ...
          if (
            bt.refType === "branch" && bt.getNameWithoutRemote() === currentName
          ) {
            return {
              name: bt.name,
              compactName: bt.getRemoteName(),
              type: bt.refType,
            };
          }
        }
    ...
    return (
      sha: string,
      options?: { compact?: boolean; icons?: boolean },
    ): string | undefined => {
      ...
      if (branchesAndTags.length > 1) {
        ...
        return `${
          options?.icons
            ? `${bt.type === "tag" ? "$(tag)" : "$(git-branch)"} `
            : ""
        }${bt.compactName ?? bt.name}, ${GlyphChars.Ellipsis}`;
      }
  ...
  @log<GitProviderService["getDiffForFileContents"]>({
    args: { 1: "<contents>" },
  })
  /**
  ...
  async getMergeStatus(
    repoPath: string | Uri,
  ): Promise<GitMergeStatus | undefined> {
    const { provider, path } = this.getProvider(repoPath);
  ...
  async getRebaseStatus(
    repoPath: string | Uri,
  ): Promise<GitRebaseStatus | undefined> {
    const { provider, path } = this.getProvider(repoPath);
  ...
  getPreviousComparisonUrisForLine(
    ...
    const { provider, path } = this.getProvider(repoPath);
    return provider.getPreviousComparisonUrisForLine(
      path,
      uri,
      editorLine,
      ref,
  ...
  @debug<GitProviderService["getMyPullRequests"]>({
    args: { 0: (remoteOrProvider) => remoteOrProvider.name },
  })
  ...
  @debug<GitProviderService["getMyIssues"]>({
    args: { 0: (remoteOrProvider) => remoteOrProvider.name },
  })
  async getMyIssues(
    remoteOrProvider: GitRemote | RichRemoteProvider,
  ...
  async getBestRemoteWithProvider(
    ...
    const remotes = await this.getBestRemotesWithProviders(
      repoPath,
      cancellation,
    );
    return remotes[0];
  ...
  async getBestRemotesWithProviders(
    ...
    if (remotes == null) {
      async function getBest(this: GitProviderService) {
        const remotes = await this.getRemotesWithProviders(repoPath, {
          sort: true,
        }, cancellation);
        ...
        const currentBranchRemote = (await this.getBranch(remotes[0].repoPath))
          ?.getRemoteName();
        ...
        for (const remote of remotes) {
          ...
          if (weight > 0 && weight < 1000 && !originalFound) {
            ...
            if (
              ...
              (p.maybeConnected ||
                (p.maybeConnected === undefined && p.shouldConnect &&
                  (await p.isConnected())))
            ) {
              if (cancellation?.isCancellationRequested) {
                throw new CancellationError();
        ...
        weighted.sort((
          [aw, ar],
          [bw, br],
        ) => (bw === 0 && aw === 0 ? sortCompare(ar.name, br.name) : bw - aw));
  ...
  async getRemotesWithProviders(
    ...
    return remotes.filter((r: GitRemote): r is GitRemote<RemoteProvider> =>
      r.provider != null
    );
  }
  ...
  async getRemotesWithRichProviders(
    ...
    const remotes = await this.getRemotes(repoPath, options, cancellation);
    return remotes.filter((r: GitRemote): r is GitRemote<RichRemoteProvider> =>
      r.hasRichIntegration()
    );
  }
  ...
  getBestRepository(
    ...
    return (editor != null
      ? this.getRepository(editor.document.uri)
      : undefined) ?? this.highlander;
  }

  getBestRepositoryOrFirst(): Repository | undefined;
  // eslint-disable-next-line @typescript-eslint/unified-signatures
  getBestRepositoryOrFirst(
    uri?: Uri,
    editor?: TextEditor,
  ): Repository | undefined;
  // eslint-disable-next-line @typescript-eslint/unified-signatures
  ...
  getBestRepositoryOrFirst(
    ...
    editor = editorOrUri ?? editor ?? window.activeTextEditor;
    return (
      (editor != null ? this.getRepository(editor.document.uri) : undefined) ??
        first(this._repositories.values())
    );
  }
  ...
  async getOrOpenRepository(
    ...
    const detectNested = options?.detectNested ??
      configuration.get("detectNestedRepositories", uri);
    if (!detectNested) {
      ...
      if ((stats.type & FileType.Directory) !== FileType.Directory) {
        ...
        if (!options?.force && this._visitedPaths.has(getBestPath(uri))) {
          return repository;
        }
      }
    ...
    if (promise == null) {
      async function findRepository(
        ...
        if (this._repositories.count !== 0) {
          ...
          root = this._repositories.getClosest(
            provider.getAbsoluteUri(uri, repoUri),
          );
        }
        ...
        const repositories = provider.openRepository(
          root?.folder,
          repoUri,
          false,
          undefined,
  ...
  async getOrOpenRepositoryForEditor(
    editor?: TextEditor,
  ): Promise<Repository | undefined> {
  ...
  async getLocalInfoFromRemoteUri(
    ...
    for (const repo of this.openRepositories) {
      for (const remote of await repo.getRemotes()) {
        const local = await remote?.provider?.getLocalInfoFromRemoteUri(
          repo,
          uri,
          options,
        );
  ...
  async getTreeEntryForRevision(
    ...
    const { provider, path: rp } = this.getProvider(repoPath);
    return provider.getTreeEntryForRevision(
      rp,
      provider.getRelativePath(path, rp),
      ref,
    );
  ...
  async hasBranchOrTag(
    ...
    options?: {
      filter?: {
        branches?: (b: GitBranch) => boolean;
        tags?: (t: GitTag) => boolean;
      };
    },
  ): Promise<boolean> {
  ...
  async getOpenScmRepositories(): Promise<ScmRepository[]> {
    const results = await Promise.allSettled(
      [...this._providers.values()].map((p) => p.getOpenScmRepositories()),
    );
    const repositories = flatMap<
      PromiseFulfilledResult<ScmRepository[]>,
      ScmRepository
    >(
      filter<
        PromiseSettledResult<ScmRepository[]>,
        PromiseFulfilledResult<ScmRepository[]>
      >(
        results,
        (r): r is PromiseFulfilledResult<ScmRepository[]> =>
          r.status === "fulfilled",
      ),
      (r) => r.value,
    );
  ...
  @log()
  getOrOpenScmRepository(
    repoPath: string | Uri,
  ): Promise<ScmRepository | undefined> {
    const { provider, path } = this.getProvider(repoPath);
